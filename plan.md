...
overview: Add a backend abstraction so the same code can run on NumPy (default) or PyTorch when `--gpu` is passed. A single `tb_cpa/backend.py` will dispatch array operations to NumPy or PyTorch; scripts set the backend from the CLI and convert results to NumPy for matplotlib.
todos: []
isProject: false
---
...
- **Single code path**: Core `tb_cpa` modules use a shared backend object for all array creation and linalg (no duplicate implementations). When `--gpu` is used, the backend is PyTorch (CUDA if available); otherwise NumPy.
- **CLI**: Add `--gpu` to [scripts/run_all.py](scripts/run_all.py) and [scripts/quick_demo.py](scripts/quick_demo.py). Set backend before any `tb_cpa` imports that depend on it. Matplotlib continues to receive NumPy arrays via `backend.to_numpy()`.
- **Scope of changes**: New file [tb_cpa/backend.py](tb_cpa/backend.py); edits in [tb_cpa/params.py](tb_cpa/params.py), [tb_cpa/lattice.py](tb_cpa/lattice.py), [tb_cpa/kpath.py](tb_cpa/kpath.py), [tb_cpa/slater_koster.py](tb_cpa/slater_koster.py), [tb_cpa/hamiltonian.py](tb_cpa/hamiltonian.py), [tb_cpa/cpa.py](tb_cpa/cpa.py), [tb_cpa/greens.py](tb_cpa/greens.py), and both scripts. Tests remain NumPy-only unless we add an optional GPU run.

## 1. Backend module

Add **tb_cpa/backend.py** that:
...
Backend must be set **before** any code that creates arrays (e.g. at script start, before building kgrid/params). So in scripts: parse args → `set_backend(args.gpu)` → then import from `tb_cpa` (or keep imports at top and set backend at the very start of `main()` so that when we call `monkhorst_pack`, etc., backend is already set).
...
- **run_all.py** and **quick_demo.py**:
  ...
  - At the start of `main()`, call `from tb_cpa.backend import set_backend, to_numpy` and `set_backend(args.gpu)`.
...
| File                      | Action                                                                          |
| ------------------------- | ------------------------------------------------------------------------------- |
| `tb_cpa/backend.py`       | **New**: backend API + NumPy/PyTorch implementations, `set_backend`, `to_numpy` |
| `tb_cpa/params.py`        | Use backend for `onsite_matrix` (and any array creation)                        |
| `tb_cpa/lattice.py`       | Use backend for all array/linalg ops                                            |
| `tb_cpa/kpath.py`         | Use backend for arrays and norm                                                 |
| `tb_cpa/slater_koster.py` | Use backend for matrix construction                                             |
| `tb_cpa/hamiltonian.py`   | Use backend for all matrix/exp/dot/kron ops                                     |
| `tb_cpa/cpa.py`           | Use backend for linalg and array creation                                       |
| `tb_cpa/greens.py`        | Use backend for linalg and array creation                                       |
...
No changes to `tb_cpa/__init__.py` or test logic required for basic behavior; tests keep using default NumPy backend.